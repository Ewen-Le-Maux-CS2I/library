...
class ExemplaireControllerTest {
    ...
    void doitAjouterExemplaireAUnOuvrage() {
        ...
        var resp = restTemplate.postForEntity(
                ...
                Map.of("cote", "SF-001"), Map.class);

        assertEquals(HttpStatus.CREATED, resp.getStatusCode());
        assertNotNull(resp.getBody(), "Le body ne doit pas être null");
    ...
    void doitListerExemplairesParOuvrage() {
        ...
        restTemplate.postForEntity("/api/ouvrages/" + livreId + "/exemplaires",
                Map.of("cote", "SF-002"), Map.class);
        restTemplate.postForEntity("/api/ouvrages/" + livreId + "/exemplaires",
                Map.of("cote", "SF-003"), Map.class);

        // ParameterizedTypeReference pour désérialiser List<Map> correctement
    ...
    void doitRetourner404SiOuvrageInexistant() {
        var resp = restTemplate.postForEntity(
                "/api/ouvrages/99999/exemplaires",
                Map.of("cote", "XX-000"), Map.class);
        assertEquals(HttpStatus.NOT_FOUND, resp.getStatusCode());
    }

    private Long creerLivre(String titre, String auteur) {
        var resp = restTemplate.postForEntity("/api/livres",
                Map.of("titre", titre, "auteur", auteur, "genre", "Roman"), Map.class);
